...
def parse_args():
    ...
    parser.add_argument('--train_path', type=str, default=os.path.join(pwd_path, '../macbert/csc_sample/train_final_pure.json'),
                        help='train dataset')
    parser.add_argument('--test_path', type=str, default=os.path.join(pwd_path, '../macbert/csc_sample/new_test.json'),
                        help='test dataset')
    parser.add_argument('--save_dir', type=str, default='./output/t5_final/', help='save dir')
    parser.add_argument('--model_name_or_path', type=str, default='/home/qian/zhouxu/work_space/pycorrector/pycorrector/t5/output/t5_better', help='pretrained model')
    ...
    parser.add_argument('--epochs', type=int, default=6, help='train epochs num')
    parser.add_argument('--max_steps', type=int, default=100000, help='train max steps')
...
class CscDataset(object):
    ...
    def load(self):
        ...
        for item in self.data:
            if 'correct_text' not in item.keys():
                print(item)
                count += 1
            else:
                data_list.append(str(item['original_text']) + '\t' + str(item['correct_text']))
        print(count)
...
def train():
    ...
    # )
    trainer.train(resume_from_checkpoint='/home/qian/zhouxu/work_space/pycorrector/pycorrector/t5/output/t5_final/checkpoint-30000')
